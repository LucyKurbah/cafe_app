...
class _ConferenceScreenState extends State<ConferenceScreen> {
  ...
  void _updateSecondTimePicker(TimeOfDay newTime) {
    ...
    setState(() {
      ...
      checkDateTimeAvailability(
          _conferenceList[0].id, _selectedtimeFrom.text, formattedTime);
    });
  }
  ...
  Future<void> addCart(id, String totalPrice, String date, String timeFrom,
      String timeTo) async {
    userId = await getUserId();
    ...
    ApiResponse response = await addConferenceToCart(
        id, totalPrice, bookDate, time_from.toString(), time_to.toString());
    if (response.error == null) {
              ...
              child: Stack(
                children: [
                  ...
                  Container(
                      ...
                      child: Padding(
                        padding:const EdgeInsets.only(left: 25, right: 40, top: 40),
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                                _conferenceList.isNotEmpty
                                    ? "PRICE: Rs. ${_conferenceList[0].price} /hr"
                                    : "",
                                style: TextStyle(
                                    fontSize: 18,
  ...
  void calculateHours(String timeString1, String timeString2) {
    ...
    print(
        'The difference between $timeString1 and $timeString2 is $hours hours and $minutes minutes');
    if (minutes > 0) {
      hours += 1;
    } else if (hours == 0) {
      hours = 1;
  ...
  void checkDateTimeAvailability(id, String timeFrom, String timeTo) async {
    ApiResponse response =
        await checkConferenceHallDetails(id, timeFrom, timeTo, _date.text);
    if (response.error == null) {
      if (response.data != null) {
        if (response.data.toString() == "VE") {
          ...
          print("Conference hall is not available on this date");
          showSnackBar(
              title: '',
              message: 'Conference hall is not available on this date');
          setState(() {
            checkTable = false;
          });
        }
