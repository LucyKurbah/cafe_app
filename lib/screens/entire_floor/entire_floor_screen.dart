...
class EntireFloorScreen extends StatefulWidget {
  const EntireFloorScreen({super.key});
...
class _EntireFloorScreenState extends State<EntireFloorScreen> {
  ...
  Future<void> addCart(int floor_id, String date) async {
    if (floor_id == 0) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Choose a floor")));
    }
    userId = await getUserId();
    ...
    if (response.error == null) {
      if (response.data == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content:
                Text("Selected Floor is not available for the selected date")));
      }
    } else if (response.error == ApiConstants.unauthorized) {
      logoutUser();
      Navigator.of(context).pushAndRemoveUntil(
                                    ...
                                    onChanged: (String? newValue) {
                                      ...
                                      setState(() {
                                        ...
                                        if (newValue == "0") {
                                          ...
                                          f_price.text =
                                              ""; // Set the price field to empty or any other default value
                                          f_id =
                                              0; // Set the floor ID to null or any other default value
                                        } else {
                                          for (var floor in _floorList) {
                                            if (floor.id.toString() ==
                                                newValue) {
                                              f_price.text =
